...
# using string variables for file download for easily way
fileName <- "UCIdata.zip"
url <- "http://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip"
dir <- "Hu Reco Using SPhones Dataset"
...
## Reading Data and save it in a variable
test <- read.table("UCI HAR Dataset/test/subject_test.txt")
train <- read.table("UCI HAR Dataset/train/subject_train.txt")
firstX_test <- read.table("UCI HAR Dataset/test/X_test.txt")
firstX_train <- read.table("UCI HAR Dataset/train/X_train.txt")
firsty_test <- read.table("UCI HAR Dataset/test/y_test.txt")
firsty_train <- read.table("UCI HAR Dataset/train/y_train.txt")
activity_labels <- read.table("UCI HAR Dataset/activity_labels.txt")
features <- read.table("UCI HAR Dataset/features.txt") 
# 1. Merges the training and the test sets to create one data set.
dataSet <- rbind(firstX_train,firstX_test)
...
activity <- rbind(firsty_train, firsty_test)
